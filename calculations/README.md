...
| `hardware_manifest.py` | **MP-03 M1 candidate hardware/power contract** — owns the 6S-R/CLEAN equipment baseline and separate 8S study overlay; distinguishes named parts, bounded classes and reserved envelopes; exports JSON and verifies configuration mass, rail capacity and the published manifest | `docs/17`, MP-04–MP-07 | stdlib plus existing battery/power contracts |
| `hardware_candidate_trade.py` | **I-33/MP-04 procurement screen** — converts primary manufacturer motor, ESC and propeller data into repeatable P42A voltage, Kv/RPM, explicit ESC-headroom, mass and envelope checks; exports the controlled 6S and study-only 8S shortlist without claiming a flight selection | I-33, H03–H07, D2/E3 | stdlib; battery and propulsion contracts |
| `hardware_measurements.py` | **MP-04 H01–H22 evidence contract** — owns every required measured field, specimen/evidence metadata and the synchronized blank JSON record; accepts pending planning data but refuses M1 closure until every physical gate is accepted | `tests/MP04-hardware-characterisation`, M1/MP-05/06 | stdlib; hardware manifest |
| `generate_hardware_dummies.py` | **MP-04 packaging-fixture generator** — emits and checks selectable OpenSCAD external-envelope shells with open ballast cavities from the controlled manifest and propulsion shortlist; explicitly excludes airworthiness, connector, cooling and tolerance authority | `geometry/equipment-dummies`, H01–H22, MP-06 | stdlib; manifest and candidate trade |
...
| `mutation_test.py` | **Proof that the suite can fail** — seeds 24 deliberate defects (missing hardware/governance records, sign flips, dropped normalisations, desynchronised copies and defeated OML predicates) and requires each to turn at least one contract check red (C43) | Whole system, CI | stdlib only |
...
python3 hardware_manifest.py             # MP-03 candidate hardware/power contract
python3 hardware_manifest.py --json      # complete machine-readable export
python3 hardware_candidate_trade.py      # MP-04 motor/ESC/propeller purchase screen
python3 hardware_measurements.py --check # synchronized blank H01--H22 record
python3 generate_hardware_dummies.py --check # generated OpenSCAD envelope shells
