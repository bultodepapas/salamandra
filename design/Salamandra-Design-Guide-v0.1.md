...
3. ADR-0040…ADR-0045, I-21/I-22/I-27 and the companion justification for rationale;
...
| CLEAN / V1 analytical mass | 1559.25 / 1602.26 g | **1553.25 / 1595.80 g** |
| CLEAN battery solution | −341.4 mm (old component model) | **−337.74 mm**, within −371.20…−336.10 mm travel |
| V1 exact battery solution | not separately exposed | **−375.48 mm; 4.28 mm beyond forward travel, CG still in band** |
...
| All-up weight (6S1P P42A) | **1553.25 g CLEAN / 1595.80 g V1 lower model**; both analytically satisfy C16 pending measured mass (§7.1) | ADR-0043/0045; `mass_budget.py` |
| Wing loading (6S1P) | **55.1 / 56.6 g/dm²** CLEAN/V1 lower model | `[D]` |
| Cruise speed / CL | **95 km/h** / CL **0.1267 CLEAN / 0.1302 V1** | `design_config.py` `[D]` |
| Stall speed | **≤45 km/h required**; **44.1 CLEAN / 44.7 V1 lower model** | ADR-0043/0045; §11 |
...
| Cnβ total | **−0.0006…−0.0014 /deg — negative** (statically unstable yaw `[E]`) | **V1a power-on −0.00029…+0.00119 /deg; power-off −0.00057…+0.00087; nominal powered +0.0005.** V1b power-on +0.00017…+0.00173; power-off −0.00020…+0.00130 `[D]` on independent `[E]` corners (I-20) |
...
| Fin geometry (V1a) | — | S_v = **2.1025 dm²**; vertical-TE trapezoid **b_v = 251.1 mm, c_r = 104.6, c_t = 62.8 mm**, AR_v = 3.0, taper 0.60, derived Λc/4 = **7.125°**; root t ≥ **3.0 mm solid** (σ 29.5 MPa, FS 1.69 at the 180 km/h structural case without spar credit); fin AC **x = +285 mm** (l_v = 379 mm from CG −93.8); root LE/TE x = +244.4/+349.1 mm; carrier extension **84.1 mm** beyond the current x = +265 pod `[D]`/`[I]` |
| **Fin section** | — | **Symmetric biconvex plate**: local maximum thickness t_v(y) = **3.0 mm root → 1.5 mm tip**, linear; TE ≈ 0.8 mm; no camber. The Ø3 mm Al rod is an **external leading-edge nose element** captured by a provisional open rear-facing Ø3.2 mm C-seat; no enclosed Ø3.2 mm bore is claimed inside the thinner plate. Root interface envelope 104.6 × 3.0 mm + 1× Ø1.75 mm filament dowel + 1× M2 screw; hole positions remain open (ADR-0038). |
| Fin mass | — | Allocation target **≤36.72 g**, but the re-derived lower model is **36.85 g shell/mount + 5.70 g spar = 42.55 g**. The **5.83 g gap is open**, and the 84 mm carrier mass is not closed; weigh the complete assembly. |
| Drag / energy | — | ΔCD0 ≈ **+0.0015** → **+10.2 % drag ≈ 1.27 Wh/km** `[E]` |
| V_stall impact | **44.1 km/h** at 1553.25 g | **44.7 km/h** at the 1595.80 g lower model; analytical pass pending F2 |
...
Installation (V1): the fin mounts on the rear-pod extension, centreline. The screening
model now separates a whole-fin equivalent powered case η = 1.25 from the motor-off
free-stream case η = 1.00; the real partial-disk wake distribution remains an F2/E8 input.
Use **1× Ø1.75 mm filament alignment dowel + screw** (ADR-0039 practice,
...
balance table of §7.1 beyond its own mass properties. The corrected component centroids
require battery x = −375.48 mm, 4.28 mm beyond current travel; the stopped solution stays
inside the released CG band. Full analysis: I-20, `yaw_stability.py`.
...
| **Nose boom (battery)** | Aluminium tube **Ø8 / int Ø6 (wall 1.0 mm)** from the CORE support at x ≈ −132 to the forward cradle plane at **x = −452.70 mm** (support span **320.7 mm**, plus 50 mm CORE insertion) + printed cradle. The pack sits between the supports. The FPV camera centre is **x = −445.98 mm, y = 0**; its lens face is the forward plane and points along −x. The printed front skid remains the crush zone. | CLEAN exact pack station **x = −337.74 mm**. V1 requires −375.48 mm, 4.28 mm beyond current travel, while remaining inside the CG band. Corrected multi-load two-support check remains provisional and shall be rerun after CAD mass properties (`boom_flexion.py`). Hybrid assembly allocation remains provisional. |
| **Battery cradle** | Printed cradle, 2 halves, walls 1.2 mm, mass ≤ 15 g: **201 mm overall length** with a calculated usable longitudinal bay of ≈ **198.1 mm**, inner cross-section **68 × 25 mm**, lower Ø8.2 channel gripping the tube; **2× 12 mm velcro straps + spring-lock hatch**; camera mount on the forward support piece. | E01 max envelope **153.0 × 65.7 × 22.6 mm**; 2.3 mm total lateral and 2.4 mm total vertical clearance; pack-centre travel **−371.20…−336.10 mm**. PROVISIONAL until F2. |
...
| **Fin mount (V1 variant, ADR-0038)** | Optional carrier from the current rear-pod end x = +265 to the calculated fin-root TE x = **+349.1 mm** (**84.1 mm extension**): fixed centreline fin S_v **2.1025 dm²**, b_v 251.1 mm, c_r 104.6 / c_t 62.8 mm, Λc/4 7.125°, root t **3.0 mm solid**, fin AC +285 mm; no rudder. The Ø3 mm aluminium rod forms the external LE nose; total root EI ≈ **1.60×**, 5.7 g, but no strength credit is taken. Carrier OML/mass, axial prop clearance and hardware positions remain open. | I-20 / `yaw_stability.py` / SLM-FIN-001; PROVISIONAL until F2 |
...
| **FPV camera mount** | The Article #1 **DJI O4 Air Unit camera (13.44 × 12.36 × 16.50 mm L×W×H)** is fixed at the foremost centreline station: centre **(x, y, z) = (−445.98, 0, −5.0) mm**, lens face flush at **x = −452.70 mm**, optical direction **−x**. E19 VTX centre is aft at **(−418.0, 0, +31.5) mm**. Their centre-to-centre straight-line lower bound is **45.99 mm**, within the **50 mm** coax length; connector exit geometry, bend radius and service routing remain open and the cable base shall not be forced. The optional Pro camera is **25.55 × 20 × 23.30 mm** with a 130 mm coax and requires a different cavity check. | Body/cable data I-19 §2/§6 `[M]`; installation relation from `balance_cg.py`/`equipment_layout.py` `[D]`; route shape `[I]`; PROVISIONAL until F2 |
...
> model is **36.85 g shell/mount + 5.70 g spar = 42.55 g**. With two servos the connected
> V1 estimate is **1595.80 g, 56.6 g/dm² and Vstall 44.7 km/h**. It remains about 24.6 g
...
> **OP-01 computational resolution updated by ADR-0045:** the reachable-CG analysis
...
> −371.20…−336.10 mm). V1 requires −375.48 mm and clamps at −371.20 mm; its calculated
> CG is −93.08 mm, within the released band but not at the exact target. The forward support is x = −452.70 mm, approximately 321 mm
...
| Cradle position | CLEAN solution centres the pack at **x = −337.74 mm** inside **−371.20…−336.10 mm** travel. V1 requires **−375.48 mm** and clamps at the forward stop; its CG remains in band but exact closure is open. | `equipment_layout.py` `[D]`; F2 |
...
| Pack configs | **Article #1 is 6S1P only**: P42A 445 g at x = −337.74 mm CLEAN; V1 uses the forward stop pending F2. A 4S aircraft needs a separate ~713 Kv motor module at the current O1 boundary and its own carrier/CG solution; both 2P packs need a different outer carrier. | ADR-0042/0045, I-16, `equipment_layout.py` `[D]` |
...
> CLEAN is **1553.25 g → 44.1 km/h**. The two-servo V1 allocation target is
> **1589.97 g → 44.6 km/h**, and the complete-fin lower model gives
> **1595.80 g → 44.7 km/h**. The exact 45 km/h mass ceiling is 1620.4 g. Binding CLEAN
...
> 44.7 km/h at the 1595.80 g V1 lower model) with
