...
> **ADR-0047 hold:** the r1/+3 deg wash-in/8%-SM rationale below records the v0.24
> history; it is not current whole-envelope trim authority. The operating-`Cm(CL)`
> redesign selects r2a at 5% nominal SM for E2A physical coupons only.
...
| Twist ε = +3.0° printed wash-in | Historical r1 cruise screen gave −0.14°/+0.50°; ADR-0047 invalidates that as whole-envelope closure and retains +3° only in the r2a E2A candidate | ADR-0041/0045/0047 `[D]`/`[I]` |
...
4. **Closed computationally for CAD by ADR-0041/0045:** at the connected V1 lower mass, the corrected r1
   root/tip integral leaves −0.14° to +0.50° neutral elevon over Ncrit 10–12, inside
   the ±0.6° cap. Actuator authority is
   verified; E2 retains physical polar/stall acceptance (`elevon_authority.py`, §5.3).

### 3.3 ADR-0043/0045 authoritative Article #1 balance (updated 2026-08-18)
...
mass: **1553.25 g CLEAN** and **1601.98 g coupled V1**, including the 48.73 g
LW-PLA-HT fin/support module and no forward-support addition. CLEAN reaches the exact
target at pack x = −337.74 mm. V1 retains the nose and converges at pack x = −363.27 mm and
target xCG −93.784 mm. Both masses analytically pass C16; F2 must verify complete-aircraft
mass, support stiffness and travel.
...
| Elevon span **35–90 % half-span** | I-27/ADR-0045 compares retained, shorter, tip-extended and chord variants. y = 227.5…585 mm retains 94.5 % of the former rigid-VLM roll derivative while removing 10.0 % moving area. The 32.5 mm fixed PANEL-root bridge separates the hinge termination from the removable joint; its structural benefit remains `[I]`. The fixed 65 mm tip avoids unnecessary low-Re/tip-vortex and aeroelastic exposure. | `[D]`/`[I]` |
| Elevon mass balance allocation **27 g/elevon** | Area-scaled moving estimate 22.5 g/elevon with the ADR-0025 1.2× balance allocation. The actual installed lead is determined from the measured printed-surface hinge moment. Mandatory; no flutter-speed improvement is claimed. | `[D]`/`[E]` |
| One actuator per elevon, 2 servos | ADR-0026/0045: one DS-939MG at y = ±406.25 mm carries the factored 180 km/h demand with 1.52× catalog margin and ≈4.5× at the initial 105 km/h limit. Flutter credit remains prohibited pending measured stiffness and G7 modal evidence. | `[D]`/`[E]` |
...
**Control-surface calculation.** `elevon_sizing.py` applies ideal thin-airfoil
effectiveness `tau = 0.6408` for the 0.28 c plain flap and fractional span-panel overlap
to an 80×6 rigid VLM. The selected surface gives `Cm_delta = +0.001828/physical deg`,
Ncrit-12 trim +0.500°, `Cl_delta_a = 0.3275/rad` and `Cl_p = −0.4160/rad`. A 5°
differential screen gives 0.72 s to 45° bank at 45 km/h `[D]/[E]`. Gap leakage,
separation, TPU compliance, freeplay and aeroelastic reversal are absent, so E2/E5/G7
remain controlling and no flap/flaperon mode or final throw is released.
...
| Value | Basis | Tag |
|---|---|---|
| PETG shell **≤550 g** | Binding CAD cap: CORE 150.1 + wings 314.8 including fixed root bridges + tips 40.0 + moving elevons 45.0. The shortened moving PETG becomes fixed panel material, so it does not reduce total shell mass. | ADR-0043/0045 `[E]` |
...
| Article #1 **1553.25 g CLEAN / 1601.98 g coupled V1** | `equipment_layout.py`; V1 includes 35.61 g LW-PLA-HT twin shells/mounts + 10.07 g spars + 3.04 g root supports and no forward-support addition. V1 remains about 18.4 g below the exact C16 ceiling. | ADR-0038/I-30 `[D]`/`[E]` |
...
- Cruise CL at the coupled V1 model: CL = 2W/(ρV²S) = **0.1318** at 95 km/h.
- Stall speed: V = √(2W/(ρ·S·CL_max)) with wing CL_max = 0.589 (I-07). The exact
  ...
  coupled V1 model is **1601.98 g → 44.74 km/h**, about 18.4 g below the ceiling. Measured mass and E2
...
| Current cradle | CLEAN/V1 **201.0 mm overall**, inner cross-section **68 × 25 mm** around E01 max **153.0 × 65.7 × 22.6 mm**; CLEAN/V1 pack centres −337.74/−363.27 mm. No V1 extension is required. | `[D]`/`[E]`/`[I]` |
...
9. **I-29 — twin-fin directional architecture correction** (`research/I-29-twin-fin-architecture-correction.md`) — current justification of §4.4; **`calculations/yaw_stability.py`**. I-20 retains the superseded centreline concept as history.
10. **ADR-0038 — dual directional configuration** (`decisions/ADR-0038-fixed-fin-variant.md`); NASA X-48 and TBS Chupito/Mojito primary sources `[M]` (I-29)
11. **I-27 — Article #1 elevon span, chord and tip-clearance trade**
    (`research/I-27-elevon-geometry-trade.md`); **ADR-0045**
    (`decisions/ADR-0045-article-1-elevon-geometry.md`); executable model
    `calculations/elevon_sizing.py`.

## 11. Directional stability and the twin-fin variant (I-29, ADR-0038)
...
| Finless yaw divergence τ ≈ 0.12 s (worst); V1a −0.952 ± 5.239i s⁻¹ | Corrected 2-DOF (β, r) dimensionalization includes the mandatory `1/(2V)` conversion for Cnr/Cyr (C31), evaluated at the single-source `I_zz` = 0.1587 kg·m² `[D]` from the 3-D mass model. V1a is a damped reduced β-r oscillation with ≈1.1 s decay, damped across the whole ±15 % inertia band; it is not a full Dutch-roll identification. | `[E]` |
| Fin CLα ≈ 0.045–0.053/deg | Helmbold-Diederich AR_v 2.0, derived Λc/4 15.064° × low-Re factor 0.85–1.00 | `[D]`/`[E]` |
| V1a total S_v = **6.1437 dm²** → Cnβ −0.00029…+0.00119 (nominal +0.0005) | Two CORE-rooted fins wholly forward of the fixed propeller hazard; each 247.9 mm span, 170.9/76.9 mm chords, taper 0.45, LE 20° and Λc/4 15.064°. No slipstream benefit is credited. Lower corner remains negative | `[D]`/`[E]` |
| V1b total S_v = **8.18 dm²** → Cnβ +0.00017…+0.00173 (nominal +0.0010) | Two swept trapezoids; positive independent-corner screen but print/mass/stall closure open | `[D]`/`[E]` |
| **Movable rudder deferred** | \|Cnδr\| ≈ 0.00053/deg (τ 0.32) cannot hold a 20 km/h crosswind slip at stall (δr ≈ 22.7° > ±20°); differential elevons are approximately 7× weaker. A rudder remains a future control variant, not part of the passive-stability baseline | `[D]`/`[E]` |
| Fin root t ≥ **3.0 mm** | Per-fin 180 km/h screen: F 47.0 N, centroid h 108 mm, M 5.09 N·m; 3.0 mm gives **FS 2.52** without spar credit; mode ≈ 8.2 Hz | `[D]` |
| Twin-fin allocation **≤60.00 g**; lower model **48.73 g** | 35.61 g two LW-PLA-HT shells/mounts + 10.07 g two spars + 3.04 g root supports. F2 must verify foamed coupons and final fillets/saddles | `[E]` |
| ΔCD0 ≈ +0.0034 (V1a) → +23.1% | Total wetted/reference area uses both calculated fins; Wh/km closure remains E2 work | `[D]`/`[E]` |
| V_stall V1a **44.74 km/h — analytical PASS** | 1601.98 g coupled model versus exact C16 mass ceiling 1620.4 g; measured F2 mass and battery/support geometry remain mandatory | ADR-0038/I-30 `[D]` |
