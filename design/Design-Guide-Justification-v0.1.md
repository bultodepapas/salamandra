...
| Twist ε = +3.0° printed wash-in | Corrected Salamandra r1 root/tip moment integral plus VLM gives −0.06°/+0.39° neutral elevon at Ncrit 10/12, inside the ±0.6° cap | ADR-0041, I-15 §8 `[D]` |
...
4. **Closed for CAD by ADR-0041:** the corrected r1 root/tip integral leaves −0.06° to
   +0.39° neutral elevon over Ncrit 10–12, inside the ±0.6° cap. Actuator authority is
...
The coupled hybrid estimate is 37.4 g. ADR-0043 budgets 1583.5 g CLEAN and 1620.2 g V1;
F2 must verify the real boom and complete-aircraft masses.
...
| Boom structure **37.4 g** | 327 mm support span + 50 mm CORE insertion, Al Ø8/int6 + printed cradle. Current two-support check: 54 MPa, FS 5.08, δ 1.4 mm, 27.0 Hz | `[D]`/`[E]` |
...
| Article #1 **1583.5 g CLEAN / 1620.2 g V1** | `mass_budget.py`; V1a fin cap 36.72 g matches the calculated lower mass bound | ADR-0043 `[D]`/`[E]` |

## 7. Propulsion

| Value | Basis | Tag |
|---|---|---|
| APC-E 8×8 (P/D 1.0) | Peak η is retained only for comparison. Solving the UIUC curve at O1 power gives J 0.899, 8,667 rpm, 2.42 N and η_prop 0.688 | ADR-0042 `[M]`/`[D]` |
| Motor 28-class, 500–550 Kv, 6S | Required rpm is 71–78 % of nominal no-load. The same motor on 4S cannot reach 8,667 rpm unloaded; a 4S module needs ~730 Kv at an estimated 80 % loaded ratio | ADR-0042 `[D]`/`[E]` |
...
- Stall speed: V = √(2W/(ρ·S·CL_max)) with wing CL_max = 0.589 (I-07). The exact
  45 km/h mass ceiling is **1620.4 g**. ADR-0043 allocates **1583.5 g CLEAN →
  44.5 km/h** and **1620.2 g V1 → 45.0 km/h**. CLEAN has 36.9 g allocation margin;
  V1 is a weighed acceptance-limit configuration. The measured E2 CLmax still governs
  flight release.
...
9. **I-20 — directional (yaw) stability and the centreline-fin variant** (`research/I-20-yaw-stability-centerline-fin.md`) — full justification of §4.4 of the guide; **`calculations/yaw_stability.py`** (nine validation cases)
...
| Yaw divergence τ ≈ 0.8 s (worst) | 2-DOF (β, r) eigenvalues, I_z band 0.23–0.33 kg·m²; current pack station −0.360 m stays inside that broad estimate | `[E]` |
...
| Fin root t ≥ **3.0 mm** | V1a at VNE: F 40.7 N, h 116 mm, M 4.72 N·m; 2.5 mm gives FS 1.16, rejected; 3.0 mm gives **FS 1.67** without crediting the Al spar; mode ≈ 8.0 Hz | `[D]` |
| Fin mass **≤36.72 g** | ADR-0043 uses the exact lower end of the current V1a estimate as a CAD acceptance cap; complete fin + spar + mount is weighed | `[E]` |
| ΔCD0 ≈ +0.0014 (V1a) → +9.8 % energy | Updated area; Wh/km ≈ 1.26 `[E]` | `[D]`/`[E]` |
| V_stall V1a **45.0 km/h** | 1620.2 g allocation; exact C16 mass ceiling 1620.4 g | ADR-0043 `[D]` |
