...
| Twist ε = +3.0° printed wash-in | At −15°, the full trade requires 3.59° equivalent with the provisional MH60→13.5 % moment. The 3.0° print cap plus 0.59° permanent elevon reflex closes it exactly inside the declared 0.6° cap. Final airfoil polars re-derive both values at C5 | I-21 `[D]` |
...
4. **Corrected by the v0.11 audit:** the favourable provisional polar leaves ≈ 0.6°
   permanent reflex, but the adverse Ncrit-12 polar leaves ≈ 1.9° and fails the cap.
   Actuator authority is verified; aerodynamic trim closure awaits the final B3 polar
   (`elevon_authority.py`, §5.3 of the guide).

### 3.3 ADR-0040 authoritative balance (2026-08-17)
...
|---|---:|---:|---:|---:|---|
| 4S1P P42A | 305 | 1545 | −500.8 | −526.1…−475.4 | No: station |
| **6S1P P42A** | **445** | **1685** | **−372.7** | **−391.7…−353.8** | **Yes** |
| 4S2P P42A | 585 | 1825 | −306.0 | −321.6…−290.4 | No one-layer envelope |
| 6S2P P42A | 865 | 2105 | −237.3 | −249.5…−225.1 | No one-layer envelope |

The cradle spans −473.3…−272.2 mm and the forward support span from x ≈ −132 is
341.3 mm; the tube includes another 50 mm of CORE insertion. The 38.2 g hybrid estimate is fixed across printed
material policies. Current stall speed is 45.9 km/h; OP-24 remains open.
...
| MH/EH families, PW51 reference | B3 screening candidates (docs/03 §3.B); PW51 is the in-service FSW airfoil of the Nemesis (I-08), **not in the UIUC database** (I-11). **C28:** the guide's "MH 45, t/c ≈ 13 %" was wrong — the aerodesign.de tailless database lists MH 45 at **9.85 % t/c, cm0 +0.0070**, documented for 15–40 g/dm² (below the project's 57 g/dm²). Provisional root candidate moved to **MH 60-12 %** (12.0 %, cm0 +0.0030). No off-the-shelf reflexed section reaches 13.5 % — R-AIRFOIL feasibility at 13.5 % is an explicit B3 question (I-11). **Screening executed (I-15 §6, `[D]`):** MH60→13.5 % cm0 = +0.0016 (Re 5e5, Ncrit 10); published cm0 values are not achieved at project Re. At −15° and 3.0° wash-in, the favourable polar needs ≈ 0.6° permanent reflex while the adverse cm0 −0.0018 polar needs ≈ 1.9° and fails the cap (`elevon_authority.py`); the designed section and final polar gate are mandatory | `[M]`/`[D]` |
...
| Boom structure **38.2 g** | 341 mm support span + 50 mm CORE insertion, Al Ø8/int6 + printed cradle. Current two-support check: 56 MPa, FS 4.96, δ 1.6 mm, 25.3 Hz | `[D]`/`[E]` |
...
| Current total **1685.2 g → 59.8 g/dm²** | `mass_budget.py`, I-16 pack, ADR-0040 boom | `[D]`/`[E]` |
...
- Stall speed: V = √(2W/(ρ·S·CL_max)) with CL_max = 0.60 (92 % of section cl_max 0.65 by
  ...
  compete, I-07 §4.2). **Current:** AUW 1685.2 g → **45.9 km/h** — over the
  requirement; declared levers: shell 550 g +
...
- **Divergence (docs/07 rev. 3, `divergence.py`):** at −15°, nominal **325.3 km/h**
  (1.36×), conservative unmeasured **128.8 km/h (0.54× — FAIL)**; GXY-plane case
  179.0; combined GXY+gyroid+1.1 mm wall 206; AERO panels 91.1. The area centroid is
...
| Cradle 155 × 66 × 24 mm | Current x envelope **−473.3…−272.2 mm**; 6S1P band −391.7…−353.8; center −372.7 | `[D]` |
| Nose boom | CORE support x ≈ −132 to forward support x ≈ −473, **341 mm span + 50 mm insertion**; aluminium **Ø8 / int Ø6** + cradle = 38.2 g. Two-support architecture mandatory | `[D]`/`[E]` |
...
| Value | Basis | Tag |
|---|---|---|
| Finless Cnβ = **−0.0006…−0.0015/deg** | Updated body length 0.738 m gives −0.00056…−0.00135 plus FSW wing 0…−0.0001. Negative across the band | `[D]`/`[E]` |
...
| V1a S_v = **2.16 dm²** → Cnβ −0.00006…+0.00096 (nominal +0.0005) | Current l_v 0.379 m; 254/106/64 mm geometry. Marginal tier does not stay positive at the full uncertainty corner | `[D]`/`[E]` |
| V1b S_v = **2.86 dm²** → +0.00048…+0.00142 (nominal +0.0010) | 293/122/73 mm; robust tier stays positive across the declared band | `[D]`/`[E]` |
...
| Fin root t ≥ **3.0 mm** | V1a at VNE: F 41.3 N, h 117 mm, M 4.81 N·m; 2.5 mm gives FS 1.15, rejected; 3.0 mm gives **FS 1.65** without crediting the Al spar; mode ≈ 7.9 Hz | `[D]` |
| Fin mass 37–62 g | V1a distributed-thickness estimate; V1b 49–82 g | `[E]` |
| ΔCD0 ≈ +0.0015 (V1a) → +9.9 % energy | Updated area; Wh/km ≈ 1.26 `[E]` | `[D]`/`[E]` |
| V_stall V1a ≈ 46.6 km/h nominal | +50 g over current 1685 g baseline | `[D]` |
